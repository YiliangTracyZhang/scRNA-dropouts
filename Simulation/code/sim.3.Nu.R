...
for(j in 1:5){
        ...
        for(i in 1:100){
                ...
                Y = cbind(Y,l)
                 # write.table(Y, paste0('/Users/kexuanliang/documents/singlecell/simulation/alterNu.ct/', Nu1, '/read', i, '.txt'), 
                            #  quote = F, col.names = F, row.names = F)
                write.table(Y, paste0('/home/kl764/project/singlecell/simulation/alterNu.ct/',  Nu1, '/read', i, '.txt'), 
                           quote = F, col.names = F, row.names = F)
                write.table(Y, paste0('/home/kl764/project/singlecell/simulation/alterNu.ct/',  Nu1, '/R', i, '.txt'), 
                            quote = F, col.names = F, row.names = F)
        }
}
