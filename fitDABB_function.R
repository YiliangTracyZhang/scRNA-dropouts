...
b_post_loglike <- function(Y, b_vec, mu_mat, bio_mat, RL_mat, Y_binary,
  ...
  loglike_mat <- Y * log((1e-10) + lambda_mat) - lambda_mat * Y_binary + 
    log(p_nodrop_mat * Y_binary + zero_prob) # lambda_mat * Y_binary should be lambda_mat * Z
  loglike_cell <- colSums(loglike_mat)
  b_loglike_vec <- - 1 / (2 * bat_mat %*% sigma2_Bat) * (b_vec - bat_mat %*% nu_Bat)^2
  p_weight <- Y_binary + zero_poisson / (zero_prob + Y_binary) #not defined rin the paper(or should be named qcg), Y_binary of no use
  phi_mat <- dnorm(eta_mean) #why???ηcg is independently drawn from N(πcg, 1),ztnb
